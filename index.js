...
Accounts.prototype.getEncodedTx = function getEncodedTx(tx, callback) {
  ...
  return Promise.all([
    ...
    isNot(tx.nonce)
      ? _this._apisCall.getTransactionCount(
    ...
    if (isNot(args[0]) || isNot(args[1]) || isNot(args[2]) || isNot(args[3])) {
      throw new Error(
        'One of the values "chainId", "gasPrice", "gasLimit" or "nonce" couldn\'t be fetched: ' +
          JSON.stringify(args),
      );
    }
    return encoded(
...
Accounts.prototype.mergeSign = function mergeSign(tx, signature, callback) {
  ...
  function merged(tx) {
    ...
    try {
      ...
      transaction.chainId = utils.numberToHex(tx.chainId);
      transaction.toMask = tx.toMask ? utils.utf8ToHex(tx.toMask) || '' : '';
      transaction.v = signature.v || '0x';
      // ? utils.numberToHex(Math.floor((Nat.toNumber(signature.v) - 35) / 2))
      ...
      const rawTransaction = RLP.encode([
        ...
        Bytes.fromNat(transaction.value),
        transaction.data,
        Bytes.fromNat(`0x${signature.v.toLowerCase()}`),
        Bytes.fromNat(`0x${signature.r.toLowerCase()}`),
        Bytes.fromNat(`0x${signature.s.toLowerCase()}`),
        '0x',
        '0x',
        '0x',
      ]);
...
Accounts.prototype.signTransaction = function signTransaction(
  ...
  function signed(tx) {
    ...
    try {
      ...
      var signature = Account.makeSigner(
      ...
      var rlpKnowledge = RLP.encode(['0x', '0x', '0x']);

      rawTx[10] = makeEven(trimLeadingZero(rlpKnowledge[0]));
      rawTx[11] = makeEven(trimLeadingZero(rlpKnowledge[1]));
      rawTx[12] = makeEven(trimLeadingZero(rlpKnowledge[2]));
...
Accounts.prototype.authTransaction = function authTransaction(
  tx,
  knowledgeKey,
  callback,
) {
  ...
  function proven(tx) {
    ...
    try {
      ...
      var signature = Account.makeSigner(
        Nat.toNumber(transaction.chainId || '0x1') * 2 + 35,
      )(Hash.keccak256(rlpEncoded), Hash.keccak256(knowledgeKey));
      ...
      result = {
        messageHash: hash,
        certV: trimLeadingZero(values[10]),
        certR: trimLeadingZero(values[11]),
        certS: trimLeadingZero(values[12]),
        rawTransaction: rawTransaction,
      };
    } catch (e) {
      callback(e);
  ...
  if (
    ...
    return Promise.resolve(proven(tx));
  }
  ...
  return Promise.all([
    ...
    return signed(
      _.extend(tx, { chainId: args[0], gasPrice: args[1], nonce: args[2] }),
    );
  });
};
...
Accounts.prototype.hashMessage = function hashMessage(data) {
  ...
  var preamble = '\x19APIS Signed Message:\n' + message.length;
  var preambleBuffer = Buffer.from(preamble);
  var apisMessage = Buffer.concat([preambleBuffer, messageBuffer]);
  return Hash.keccak256s(apisMessage);
};
...
function Wallet(accounts) {
  ...
  this.length = 0;
  this.defaultKeyName = 'app3js_wallet';
}
